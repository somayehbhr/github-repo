    ...
    <div className="flex flex-wrap main-cont bg-gray-100 min-h-screen p-5 ">
      ...
      <div className="w-full px-5 sm:w-1/4">
        ...
        <div>
          ...
          <span>{formatNotationNumber(repo.forks_count)}</span>
        </div>
        <div>
          <span className="font-bold">Stars: </span>
          <span>{formatNotationNumber(repo.stargazers_count)}</span>
        </div>
        <div>
          <span className="font-bold">Open issues: </span>
          <span>{formatNotationNumber(repo.open_issues_count)}</span>
        </div>
        <div>
          <span className="font-bold">Watching: </span>
          <span>{formatNotationNumber(repo.watchers_count)}</span>
        </div>
        <div className="my-3">
          <p className="text-lg font-bold mb-3">Licenses: </p>

          <div className="flex flex-wrap gap-3">
            {Object.values(repo.license ?? {}).map((item, index) => (
              <span
